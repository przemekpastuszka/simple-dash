...
dummy_component = dcc.Graph(figure=dict(
    series=[{
        "x": input_a,
        "y": input_b,
        "z": dcc.Markdown(input_d),  # this should be ignored
        "marker": ("something", {
            "color": input_c
        })
    }]
...
def test_retrieves_data_providers_and_their_accessors():
    ...
    providers_with_accessors = [(x.data_provider._dash_input, repr(x.accessor)) for x in
                                providers_with_accessors['figure']]
